...
class ImageGenerator(object):
    ...
    def init_dataloader(self):
        ...
        train_dataset = MNIST(
            "G:/data/mnist/", train=True, download=False, transform=tf
        )
        self.train_dataloader = DataLoader(
            train_dataset, batch_size=self.batch_size, shuffle=True, drop_last=True
        )
        val_dataset = MNIST("G:/data/mnist/", train=False, download=False, transform=tf)
    ...
    def train(self):
        ...
        for epoch in range(self.epoch):
            ...
            for i, (images, labels) in enumerate(self.train_dataloader):
                ...
                # Print batch loss
                print(
                    f"Epoch: {epoch+1}/{self.epoch}, Batch: {i+1}/{len(self.train_dataloader)}, Loss: {loss.item():.4f}"
                )
    ...
    def visualize_results(self, epoch):
        ...
        save_image(
            out, os.path.join(output_path, "{}.jpg".format(epoch)), nrow=image_frame_dim
        )
